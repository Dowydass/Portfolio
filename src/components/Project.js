    ...
    return (
        <main className="bg-green-100 min-h-screen p-12">
            <section className="container mx-auto ">
                <h1 className="text-5xl flex justify-center mb-12"></h1>
                <h2 className="text-lg text-gray-100 flex justify center "></h2>
                <section className="grid grid-cols-2 gap-8">
                    {projectData && projectData.map((project, index) =>(
                    <article className="relative rounded-lg shadow-xl bg-white p-8">
                        <h3 className="text-gray-800 text-3xl font-bold mb-2 hover:text-red-700"></h3>
                        <a 
                        href={project.link}
                        alt={project.title}
                        target="_blank"
